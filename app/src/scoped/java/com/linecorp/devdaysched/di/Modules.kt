...
val newsModule = module {
    scope(named<NewsActivity>()) {
        scoped<Long> { 999_999_999_999L }
        scoped<String> { getLongString() }
        scoped<BigDecimal> { BigDecimal(999_999_999_999L) }
        scoped<LargeObjects> { LargeObjects(get(), get(), get()) }
        scoped<NewsRepository> { LocalNewsRepository() }
        scoped<NewsContract.Presenter> { (view: NewsContract.View) ->
            createNewsPresenter(view, get())
        }
    }
}

private fun createNewsPresenter(view: NewsContract.View, repository: NewsRepository) =
    NewsPresenter(repository).apply {
        subscribe(view)
    }
...
val speakerModule = module {
    scope(named<SpeakerActivity>()) {
        scoped<ChuckInterceptor> { ChuckInterceptor(get()) }
        scoped<OkHttpClient> { createOkHttpClient(get()) }
        scoped<Converter.Factory> { GsonConverterFactory.create() }
        scoped<CallAdapter.Factory> { RxJava2CallAdapterFactory.create() }
        scoped<Retrofit> {
            createRetrofit(
                get(),
        ...
        scoped<SpeakerServices> { createSpeakerServices(get()) }
        scoped<SpeakerRepository> { createSpeakerRepository(get()) }
    }
}
